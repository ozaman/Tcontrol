		...
		<div  class="form-group ">
			<form class="form-horizontal " id="edit_plan_com_price<?=$val->id;?>" role="form">
				<div  class="col-md-2 ">
					...
					<button type="button" onclick="edit_plan_com_price_company('<?=$val->id;?>')" class="btn btn-md btn-info btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="แก้ไข้"><i class="fa fa-pencil"></i></button>
					<button type="button" class="btn btn-md btn-danger btn-equal" data-toggle="modal" data-target="#deleteModal"  data-original-title="ลบ" onclick="firstDelete('ประเภทค่าตอบแทน','<?=$val->id;?>','<?=TBL_SHOP_COUNTRY_COM_LIST_COMPANY;?>')"><i class="fa fa-trash-o"></i></button>
				</div>
				<div class="col-md-8">
					<?php
				...
				<div class="col-md-2">
					<button type="button" class="btn btn-success btn_show_hide btn_<?=$val->id;?> hide" id="btn_<?=$val->id;?>" onclick="save_edit_com_company('<?=$val->id;?>')" >บันทึก</button>
				</div>
			</form>
		</div>
		<?php
